...
def subseqs(s):
    ...
    if len(s)==0:
        ...
        number = s[0]
        return subseqs(s[1:])+(insert_into_all(number,subseqs(s[1:])))
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = [i for i in s if i>=prev]
            #b = 
            return insert_into_all(prev,subseq_helper(a[1:], a[0])) + subseq_helper(s[1:], s[0])
...
def make_glookup(class_assignments):
    ...
    total_over = 0
    total_under = 0
    def lookup(element,number):
        nonlocal total_over
        nonlocal total_under
        total_over = total_over + number
        total_under = total_under + class_assignments[element]
